...
export async function sendTokenToBackend(token: string, userId?: string): Promise<boolean> {
  try {
    ...
    if (!backendUrl || backendUrl === 'https://your-api.com' || backendUrl === 'https://api.example.com') {
      if (__DEV__) console.warn('Backend URL not configured. Set EXPO_PUBLIC_API_URL in your .env file to enable token registration.');
      return false;
    }
    ...
    if (!response.ok) {
      if (__DEV__) console.warn('Failed to send token to backend:', response.statusText);
      return false;
    }
